...
export async function GET(request: Request) {
	...
	let query = supabaseAgua.from('maintenance_types').select('*');
	...
	if (error) {
		return NextResponse.json(
			{
				data: error.code,
				error: 'Ha ocurrido un error al obtener los tipos de mantenimiento',
			},
			{ status: 500 },
		);
	}
...
export async function POST(request: Request) {
	...
	if (error) {
		return NextResponse.json(
			{
				data: error.code,
				error: 'Ha ocurrido un error al crear el tipo de mantenimiento',
			},
			{ status: 500 },
		);
	}
